...
Page({
  ...
  data: {
    ...
    nextSevenDays:{},
    choose_day:"",
    activity_time_duration:["09:00-12:00","16:00-18:00","18:00-20:00","20:00-22:00"],
    choose_duration:"",
  ...
  getNextSevenDays() {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (i === 0) {
            ...
            if (i <= daysUntilEndOfWeek) {
                weekLabel = `本${daysOfWeek[futureDayOfWeek]}`;
            } else {
                weekLabel = `下${daysOfWeek[futureDayOfWeek]}`;
            }
        }
        ...
        result[formattedDate] = weekLabel;
    }
...
show_time(){
  this.setData({
    show_time_flag:!this.data.show_time_flag
  })
},
choose_day(e){
  ...
  if(value!=""&&this.data.choose_duration!=""){
    var begintime = value + " " + this.data.choose_duration.split("-")[0]
    var endtime = value + " " + this.data.choose_duration.split("-")[1]
    var addendtime = value + " " + this.data.choose_duration.split("-")[0]
    var cancelendtime = datatime_util.subtractHours(value + " " + this.data.choose_duration.split("-")[0],24)
    this.setData({
      begintime,endtime,addendtime,cancelendtime
    })
    
  }
},
choose_duration(e){
  var value = e.target.dataset.duration;
  console.log(value)
  this.setData({
    choose_duration:value
  })
  if(value!=""&&this.data.choose_day!=""){
    var begintime = this.data.choose_day + " " + value.split("-")[0]
    var endtime = this.data.choose_day + " " + value.split("-")[1]
    var addendtime = this.data.choose_day + " " + value.split("-")[0]
    var cancelendtime = datatime_util.subtractHours(this.data.choose_day + " " + value.split("-")[0],24)
    this.setData({
      begintime,endtime,addendtime,cancelendtime
    })
  ...
  // 校验字符串是否符合 YYYY-MM-DD HH:MM 格式的函数
 isValidDateTime(str) {
  ...
  create_activity(nickname, url, gender) {
    ...
    if (this.data.title == "") {
      ...
      return;
    }else if(!(this.isValidDateTime(this.data.begintime)&&this.isValidDateTime(this.data.endtime)&&this.isValidDateTime(this.data.addendtime)&&this.isValidDateTime(this.data.cancelendtime))){
      wx.showToast({
        title: "请选择活动时间",
        icon: "none",
        duration: 3000
    ...
    wx.request({
      ...
      success(res) {
        ...
        setTimeout(function(){
          var activity_info = {}
          activity_info["activity_id"] = result["activity_id"]
          activity_info["activity_tag"] = that.data.hobby_tag.trim()
  ...
  onLoad: function (options) {
    console.log(this.getNextSevenDays());
  ...
  onShow: function () {
    ...
    //app.globalData.activity_more_setting = {}
  },
  more_setting(){
    wx.navigateTo({
      url: '/pages/activity/activitymore',
    })
  },
